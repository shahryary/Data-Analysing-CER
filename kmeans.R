...
test_ <- mainData[, 
                  eval(toRun),
                  by=.(ID)] # mean per  Month if you want to every day



data.p <- as.matrix(test_)
...
# Ask for user input - determine the number of cluster solutions to test (put 7 , 15 )
...
clust.level <- as.integer(choose.clust()) # input 4
